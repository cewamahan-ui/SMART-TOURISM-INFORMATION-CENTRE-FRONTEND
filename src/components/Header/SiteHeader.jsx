...
import { Menu, AlertTriangle, Bell, Heart, User, Settings, X } from "lucide-react";
...
export function SiteHeader({ transparent = false }) {
  ...
  return (
    <>
      <div
        className="pointer-events-none fixed top-5 z-30 hidden -translate-x-1/2 lg:block"
        style={{ left: "50%" }}
      >
      ...
      <header
        className={
          "absolute z-40 w-full lg:hidden " +
          (transparent
            ? "text-cream"
      ...
      <button
        className="fixed left-6 top-5 z-40 hidden items-center gap-2 rounded-full border border-current/25 bg-background/85 px-4 py-2 text-xs uppercase tracking-widest text-foreground backdrop-blur hover:bg-black/10 lg:inline-flex"
        onClick={() => setOpen(true)}
